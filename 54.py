...
# CARDS_Test = "AC 3C 3D QC QC QD 3C 3D QD 6D"

def poker_skin_num(P):
    for i in range(5):
        if P[i][0] == "T":
            P.append(10)
        elif P[i][0] == "J":
...
def poker_hand(P):
    poker_skin_num(P)
    N = [P[i] for i in range(5,10)]
    N.sort()
    P.append(N)
    if P[0]==P[1]==P[2]==P[3]==P[4]:                                    # ВСЕ МАСТИ РАВНЫ!
        if N == [10,11,12,13,14]:
            return 1000                                                 # Роял-флаш ==
        elif N[0]+4==N[1]+3==N[2]+2==N[3]+1==N[4]:
            return 900+N[4]                                             # Стрейт-флаш ==
        else:
            return 600+N[4]                                             # Флаш == ???
    else:
        if N[0]==N[1]==N[2]==N[3] or N[4]==N[1]==N[2]==N[3]:
            return 800+N[2]                                             # Каре
        elif (N[0]==N[1]==N[2] and N[3]==N[4]) or (
        N[0]==N[1] and N[2]==N[3]==N[4]):
            return 700+N[2]                                             # Фул-хаус
        elif N[0]+4==N[1]+3==N[2]+2==N[3]+1==N[4]:
            return 500+N[4]                                             # Стрейт ==
        elif N[0]==N[1]==N[2] or N[2]==N[3]==N[4] or N[3]==N[1]==N[2]:
            return 400+N[2]                                             # Тройка
        elif N[0]==N[1] and N[2]==N[3]:
            P.append(min(N[1],N[2]))
            return 300 + max(N[1],N[2])                                 # _Две пары == ???
        elif N[2]==N[1] and N[4]==N[3]:
            P.append(min(N[3],N[2]))                                    
            return 300 + max(N[3],N[2])                                 # _Две пары == ???
        elif N[0]==N[1] and N[4]==N[3]:
            P.append(min(N[3],N[1]))
            return 300 + max(N[3],N[1])                                 # _Две пары == ???
        elif N[0]==N[1]:
            return 200+N[0]                                             # _Одна пара == ???
        elif N[2]==N[1]:
            return 200+N[1]                                             # _Одна пара == ???
        elif N[2]==N[3]:
            return 200+N[2]                                             # _Одна пара == ???
        elif N[4]==N[3]:
            return 200+N[3]                                             # _Одна пара == ???
    return 100+N[4]

def poker_compare_win1(P1,P2):
    aa = list(P1)
    bb = list(P2)
    a = poker_hand(P1)
    b = poker_hand(P2)
    if a > b:
        ...
        for i in range(4,-1,-1):
            if P1[10][i] > P2[10][i]:
                # print(P1[10][i],">",P2[10][i])
                return True
            elif P1[10][i] < P2[10][i]:
                return False
    return False
...
for CARD in CARDS_list:
    Players = CARD.split()
    Player1 = [Players[i] for i in range(5)]
    Player2 = [Players[i] for i in range(5,10)]
    # a = list(Player1)
    # b = list(Player2)
    if poker_compare_win1(Player1,Player2):
        Win1 += 1
        # print(a," - ",b)
...
100+Старшая карта: Карта наибольшего достоинства.
200+Одна пара: Две карты одного достоинства.
300+Две пары: Две различные пары карт
400+Тройка: Три карты одного достоинства.
500+Стрейт: Все пять карт по порядку, любые масти.
600+Флаш: Все пять карт одной масти.
700+Фул-хаус: Три карты одного достоинства и одна пара карт.
800+Каре: Четыре карты одного достоинства.
900+Стрейт-флаш: Любые пять карт одной масти по порядку.
1000+Роял-флаш: Десятка, валет, дама, король и туз одной масти.
...
первые пять - карты 1-го игрока, оставшиеся пять - карты 2-го игрока. Можете считать, что все ставки верны (нет неверных символов или повторов карт), ставки каждого игрока не следуют в определенном порядке, и что при каждой ставке есть безусловный победитель.
